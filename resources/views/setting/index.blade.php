        ...
        @foreach ($setting as $stg)
            <div class="row col-sm-6">
                <div class="col-sm">
                    ...
                    <label for="exampleFormControlInput1">Transaksi {{ $stg->nama_transaksi }}
                    </label>
                </div>
                <div class="col-sm">
                    <label for="exampleFormControlInput1">{{ $stg->kd_akun }}</label>
                </div>
                <div class="col-sm">
                    <select name="akun[]" id="supp select2" class="form-control" required width="100%">
                        <option value="">Pilih Akun</option>
                        @foreach ($akun as $akn)
                            <option value="{{ $akn->kd_akun }}">{{ $akn->kd_akun }} - {{ $akn->nm_akun }} </option>
                        @endforeach
                    </select>
                </div>
            </div>
