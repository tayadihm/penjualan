...
<form action="/detail/simpan" method="POST">
 @csrf
   <div class="form-group col-sm-4">
       ...
       <input type="hidden" name="no_jurnal" value="{{ $formatnyaj }}" class="form-control" id="exampleFormControlInput1" >
               ...
               <select name="cust" id="cust select2"  class="form-control" required width="100%">
                   <option value="">Pilih</option>
                         @foreach ($customer as $customer)
                           <option value="{{ $cust->id_cust }}">{{ $cust->nm_cust }} - {{ $cust->nohp }} -{{ $cust->email }}  -{{ $cust->alamat }} </option>
                         @endforeach
               </select>
   </div>
...
<tr>
   ...
   <td><input name="id_cust[]" class="form-control" type="hidden" value="{{$temp->id_cust}}" readonly>{{$temp->id_cust}}</td>
   <td> <input name="subtot[]" class="form-control" type="hidden" value="{{$temp->subtot}}" readonly >{{$temp->sub_subtot}}</td>
   <td align="center">
